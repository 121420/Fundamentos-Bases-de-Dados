...
1. Entidades
As entidades identificadas são: Produto, Encomenda, Fornecedor, TipoFornecedor e ItemEncomenda (entidade associativa entre Encomenda e Produto).

2. Entidades e Atributos:
- Produto (Código / ProdutoID (Chave-Primária), Nome, Preço, Taxa de IVA, Unidades em armazem);
- Encomenda (Número de encomenda/Encomenda ID (Chave-Primária), Data de realização/Data_Encomenda, Fornecedor Único);
- Fornecedor (Nome, NIF (Chave-Primária), Fax, Condições de pagamento);
- TipoFOrnecedor (Código interno (Chave-Primária), Designação);
